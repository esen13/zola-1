...
import { canCreateAppointment } from "@/lib/schedule/permissions"
import {
  calculateEndTime,
  checkTimeConflict,
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Используем синтаксис с указанием колонки для правильного join при нескольких FK на одну таблицу
  ...
  try {
    ...
    if (!doctor_id || !patient_id || !starts_at) {
      return NextResponse.json(
        {
          error:
            "Не указаны обязательные поля: doctor_id, patient_id, starts_at",
        },
        { status: 400 }
      )
    }
